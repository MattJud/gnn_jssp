...
# -*- coding: utf-8 -*-
"""
Created on Tue Jun  1 01:12:20 2021
...
def DataExtractNorm(path_to_json, json_file, time_max, time_min, machine_max, machine_min, special_data=False):
    ...
    for data_index in range(len(new_df.index)):
        ...
        node_values = np.append(node_values, input_machine)
    
    
    # # Add node features for S and T nodes, 2 for S, 3 for T
    node_features = np.append(node_values, [2,3], axis=0)
    ...
    # =============================================================================
    # Create empty array
    edge_attr = np.zeros(len(complete_source_nodes))
    
    # Iterate using complete_source_nodes to get node number of source
    for node_num in range(S_node):
        # Get index of source node
        node_pos = np.where(complete_source_nodes == node_num)
        
        # Replace value in edge_attr with corresponding time value of the source node
        np.put(edge_attr, node_pos[0], new_df['time'][node_num])
        if node_num == S_node:
            np.put(edge_attr, node_pos[0], 0)
...
class WeightedGraph3x3(InMemoryDataset):
    def __init__(self, root, transform=None, pre_transform=None):
        super(WeightedGraph3x3, self).__init__(root, transform, pre_transform)
    ...
    def processed_file_names(self):
        return ['../input/WeightedGraph3x3.pt'] #Has to create 'input' folder if hasn't been created yet, otherwise Errno 2,no such directory will be thrown
...
class WeightedGraph6x6(InMemoryDataset):
    def __init__(self, root, transform=None, pre_transform=None):
        super(WeightedGraph6x6, self).__init__(root, transform, pre_transform)
        self.data, self.slices = torch.load(self.processed_paths[0])
    ...
    @property
    def processed_file_names(self):
        return ['../input/WeightedGraph6x6.pt'] #Has to create 'input' folder if hasn't been created yet, otherwise Errno 2,no such directory will be thrown
...
class WeightedGraph6x6L2D(InMemoryDataset):
    def __init__(self, root, transform=None, pre_transform=None):
        super(WeightedGraph6x6L2D, self).__init__(root, transform, pre_transform)
        self.data, self.slices = torch.load(self.processed_paths[0])
    ...
    @property
    def processed_file_names(self):
        return ['../input/WeightedGraph6x6L2D.pt'] #Has to create 'input' folder if hasn't been created yet, otherwise Errno 2,no such directory will be thrown
...
class WeightedGraph8x8(InMemoryDataset):
    def __init__(self, root, transform=None, pre_transform=None):
        super(WeightedGraph8x8, self).__init__(root, transform, pre_transform)
        self.data, self.slices = torch.load(self.processed_paths[0])
    ...
    @property
    def processed_file_names(self):
        return ['../input/WeightedGraph8x8.pt'] #Has to create 'input' folder if hasn't been created yet, otherwise Errno 2,no such directory will be thrown
...
class WeightedGraph10x10(InMemoryDataset):
    def __init__(self, root, transform=None, pre_transform=None):
        super(WeightedGraph10x10, self).__init__(root, transform, pre_transform)
        self.data, self.slices = torch.load(self.processed_paths[0])
    ...
    @property
    def processed_file_names(self):
        return ['../input/WeightedGraph10x10.pt'] #Has to create 'input' folder if hasn't been created yet, otherwise Errno 2,no such directory will be thrown
...
class WeightedGraph15x15(InMemoryDataset):
    def __init__(self, root, transform=None, pre_transform=None):
        super(WeightedGraph15x15, self).__init__(root, transform, pre_transform)
        self.data, self.slices = torch.load(self.processed_paths[0])
    ...
    @property
    def processed_file_names(self):
        return ['../input/WeightedGraph15x15.pt'] #Has to create 'input' folder if hasn't been created yet, otherwise Errno 2,no such directory will be thrown
...
class WeightedGraph15x15L2D(InMemoryDataset):
    def __init__(self, root, transform=None, pre_transform=None):
        super(WeightedGraph15x15L2D, self).__init__(root, transform, pre_transform)
        self.data, self.slices = torch.load(self.processed_paths[0])
    ...
    @property
    def processed_file_names(self):
        return ['../input/WeightedGraph15x15L2D.pt'] #Has to create 'input' folder if hasn't been created yet, otherwise Errno 2,no such directory will be thrown
